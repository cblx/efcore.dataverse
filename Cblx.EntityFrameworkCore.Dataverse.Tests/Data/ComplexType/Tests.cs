...
public class Tests
{
    ...
    public async Task ExceptionHandlingTest()
    {
        ...
        var thing = new Thing { Id = id, 
            Data = new ComplextData { Name = "Test" }
        };
        var id2 = ThingId.New();
        var thing2 = new Thing { Id = id2, Data = new ComplextData { Name = "Test" } };
        ...
        static async Task AddAgainWithAssertion(Thing thing, Thing thing2)
        {
            ...
            context.Things.Add(new Thing { Id = ThingId.New(), Data = new ComplextData { Name = "Test" } }); // TODO: We're not cleaning up this thing after the test.
    ...
    private static async Task CheckChildThingState(ThingId thingId, ThingId childThingId)
    {
        ...
        childThing!.Data.Name.Should().Be("Child");
        childThing.Data.ParentId.Should().Be(thingId);
    }
    ...
    private static async Task<ThingId> CreateChildThing(ThingId thingId)
    {
        ...
        var childThing = new Thing { Data = new ComplextData { Name = "Child", ParentId = thingId } };
        using var db = CreateContext();
    ...
    private static async Task CheckThingStateAfterUpdate(ThingId thingId)
    {
        ...
        thing!.Data.Name.Should().Be("Test2");
    }
    ...
    private static async Task CheckThingState(ThingId thingId)
    {
        ...
        var thing = await db.Things.FindAsync(thingId);
        thing.Should().NotBeNull();
        thing!.Data.Name.Should().Be("Test");
    }

    private static async Task<ThingId> CreateAThing()
    {
        var thing = new Thing { Data = new ComplextData { Name = "Test" } };
        using var db = CreateContext();
        db.Things.Add(thing);
        await db.SaveChangesAsync();
        return thing.Id;
